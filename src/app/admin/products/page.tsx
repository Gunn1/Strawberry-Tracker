...
import { parseCents } from "@/lib/format/money";
import type { Product } from "@/types/domain";

/** The units a product can be priced by, offered in the add/edit form. */
const UNITS = ["qt", "lb", "pint", "each", "bag", "dozen"];

/** A product's fields while they're being edited, held as strings. */
...
export default function ProductsPage() {
  ...
  // `all=1` so inactive products stay editable here, unlike on the till.
  const load = useCallback(async () => {
    try {
      const list = await api.get<Product[]>("/api/products?all=1");
      setProducts(list);
      seed(list);
    } catch (err) {
  ...
  async function addProduct(e: React.FormEvent) {
    ...
    try {
      await api.post("/api/products", {
        name: newName,
        unit: newUnit,
        priceCents: Math.max(0, parseCents(newPrice)),
      });
      setNewName("");
      setNewPrice("");
      ...
      setError(errorMessage(err, "Couldn't add that product."));
    } finally {
      setAdding(false);
    }
  }
  ...
  async function patch(id: string, body: Record<string, unknown>, msg?: string) {
    ...
    setError(null);
    try {
      const updated = await api.patch<Product>(`/api/products/${id}`, body);
      setProducts((prev) => prev.map((p) => (p.id === id ? updated : p)));
      setDrafts((d) => ({
        ...d,
        [id]: { name: updated.name, unit: updated.unit, price: (updated.priceCents / 100).toFixed(2) },
      }));
      if (msg) setToast(msg);
    } catch (err) {
      setError(errorMessage(err, "Couldn't update that product."));
    } finally {
      setBusyId(null);
    }
  }
  ...
  const saveDraft = (p: Product) => {
    const d = drafts[p.id];
    if (!d) return;
    patch(p.id, { name: d.name.trim(), unit: d.unit, priceCents: Math.max(0, parseCents(d.price)) }, "Saved");
  };

  async function remove(p: Product) {
    if (!window.confirm(`Remove "${p.name}"? Past sales keep their recorded name.`)) return;
    setBusyId(p.id);
    setError(null);
    try {
      ...
      setError(errorMessage(err, "Couldn't remove that product."));
    } finally {
      setBusyId(null);
    }
  }
