...
import type { CurrentUser, Role, StaffUser } from "@/types/domain";
...
export default function UsersPage() {
  ...
  const load = useCallback(async () => {
    try {
      // Knowing who you are is only needed to stop you removing yourself, so a
      // failure there shouldn't hide the roster.
      const [list, me] = await Promise.all([
        api.get<StaffUser[]>("/api/users"),
  ...
  async function addUser(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.post("/api/users", { email: newEmail, role: newRole });
      setNewEmail("");
      setNewRole("STAFF");
      setToast("User added");
      await load();
    } catch (err) {
      setError(errorMessage(err, "Couldn't add that user."));
    } finally {
      setAdding(false);
    }
  }
  ...
  async function patch(id: string, body: { role?: Role; active?: boolean }) {
    ...
    setError(null);
    try {
      const updated = await api.patch<StaffUser>(`/api/users/${id}`, body);
      setUsers((prev) => prev.map((u) => (u.id === id ? updated : u)));
    } catch (err) {
      setError(errorMessage(err, "Couldn't update that user."));
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function remove(u: StaffUser) {
    if (!window.confirm(`Remove ${u.email}? They'll lose access.`)) return;
    setBusyId(u.id);
    setError(null);
    try {
      ...
      setError(errorMessage(err, "Couldn't remove that user."));
    } finally {
      setBusyId(null);
    }
  }
