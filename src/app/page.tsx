...
export default function RedWagonFarm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api.get<FarmStatus & Hours>("/api/status");
        if (!active) return;
        setStatus({ openStatus: data.openStatus, statusNote: data.statusNote });
        setHours({
          hoursWindow: data.hoursWindow,
          hoursDays: data.hoursDays,
          hoursFinishBy: data.hoursFinishBy,
        });
      } catch {
  ...
  const subscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubState("error");
      setSubMsg(errorMessage(err, "Couldn't sign you up. Please try again."));
    }
  };
